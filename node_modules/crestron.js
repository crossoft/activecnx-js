   ...
   /**
    ...
    var processPacket = function(type, payload) {
        try {
			...
			for (var i = 0; i < payload.length; ++i) {
				if (i > 0) str += ' ';
				str += hex[payload[i] >> 4] + hex[payload[i] & 0x0F];
			}
            shutdown('invalid packet received (type ' +
				type + ', data ' + str + '): ' + err);
        }
    }
    ...
    this.sendDigital = function(join, value, slot) {
        ...
        payload[3 + extra] = 0;
        payload[4 + extra] = (join - 1) & 0xFF;
		payload[5 + extra] = ((join - 1) >> 8) | (value ? 0 : 0x80);
        sendPacket(DATA_PACKET, payload);
        self.emit('digitalSent', slot || 0, join, value);
    }
